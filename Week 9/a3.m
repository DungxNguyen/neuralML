...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.input_to_hid = model.input_to_hid * wd_coefficient + (model.hid_to_class' * (class_prob - data.targets)) .* (hid_output .* (1 - hid_output)) * data.inputs' ./ size(data.inputs)(2) ;
  ret.hid_to_class = model.hid_to_class * wd_coefficient + (class_prob - data.targets) * hid_output' ./ size(hid_output)(2);
end
